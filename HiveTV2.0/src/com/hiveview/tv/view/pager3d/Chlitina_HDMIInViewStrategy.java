...
@SuppressLint("NewApi")
public class Chlitina_HDMIInViewStrategy extends Abstract3DPagerStrategy {
	...
	// 首页推荐位
	private View matrix_recommend_layout_0,thumb_layout_0,matrix_recommend_layout_1, matrix_recommend_layout_3,
			matrix_recommend_layout_4;
	// 游戏右边推荐位
	private View matrix_game_layout_4, matrix_game_layout_5,
	matrix_game_layout_6;

	public Chlitina_HDMIInViewStrategy(View3DPager _3dPager,
			OnDataCompleted dataCompleted) {
		super(_3dPager);

		appView = new MatrixAppView(view3dPager.getContext(),
				new AppFocusDirectioner());
		if (null == cinemaView) {
			cinemaView = new MatrixCinemaViewNew(view3dPager.getContext(),
					new CinemaFocusDirectioner());
		}
		recommendView = new MatrixRecommendView(view3dPager.getContext(),
				new RecommendFocusDirectioner());
		gameView = new MatrixGameView(view3dPager.getContext(),
				new GameFocusDirectioner());
		educationView = new MatrixEducationView(view3dPager.getContext(),
				new EDU());
		...
		onDataComplet = new OnDataComplet() {
			...
			public void getCinemaLayout_12(int isVisible_12) {
				// TODO Auto-generated method stub
				thumb_layout_0
				.setNextFocusLeftId((isVisible_12 == View.VISIBLE) ? R.id.matrix_cinema_layout_12_new_v
						: R.id.matrix_cinema_layout_12_new_h24);
		...
		view3dPager.addTabPageView(cinemaView, 3);
	...
	class GameFocusDirectioner implements RecommendViewFocusDirectionListener {
		...
		public void setButtomMenuViewFocusDirection(View[] views) {
			...
			if (AppConstant.ISDOMESTIC) {
				gameExternalLayout
						.setNextFocusRightId(R.id.sub_navigation_common_tv_record_text_layout);
			} else {
				gameExternalLayout
			...
			if (AppConstant.ISDOMESTIC) {
				if(HomeActivity.isNeedChlitina){
					matrix_game_layout_4 = views[1];
					matrix_game_layout_5 = views[2];
					matrix_game_layout_6 = views[3];
	...
	class TvFocusDirectioner implements RecommendViewFocusDirectionListener {
		...
		public void setButtomMenuViewFocusDirection(View[] views) {
			...
			if (!AppConstant.ISDOMESTIC) {
				 ...
				 * settingView.setNextFocusLeftId(R.
				 * id.sub_navigation_recommend_user_text_layout);
				 * settingView.setNextFocusRightId
				 * (R.id.sub_navigation_recommend_record_text_layout);
				 */
				/* end by guosongsheng */
			} else {
				// 直播提醒
	...
	class RecommendFocusDirectioner implements
		...
		public void setRecommendViewsFocusDirection(View[] views) {
			/*
			 * if (null != views) { if (AppConstant.ISFOREIDGN) {
			 * views[0].setNextFocusLeftId(R.id.matrix_tv_layout_6);
			 * views[3].setNextFocusLeftId(R.id.matrix_tv_layout_7); } else {
		...
		public void setButtomMenuViewFocusDirection(View[] views) {
			...
			matrix_recommend_layout_0=views[1];
			thumb_layout_0=views[2];
	...
	class BluelightFocusDirectioner implements
		...
		public void setRecommendViewsFocusDirection(View[] views) {
			if (null != views) {
				 ...
				 * if (AppConstant.ISDOMESTIC) {
				 * views[0].setNextFocusLeftId(R.id.matrix_tv_layout_6);
				 * views[1].setNextFocusLeftId(R.id.matrix_tv_layout_7); } else
		...
		public void setButtomMenuViewFocusDirection(View[] views) {
			...
			if (AppConstant.ISDOMESTIC) {
				...
				// 外接媒体
				bluelightRecordLayout
						.setNextFocusLeftId(R.id.sub_navigation_tv_notification_text_layout);
			}
			// 影片收藏
	...
	public void setInstallApkCount(int gameCount, int appCount) {
		if(HomeActivity.isNeedChlitina){
			((MatrixRecommendView) pageViews.get(2)).setAppInstallCount(appCount
					+ gameCount);	
		}else{
		((MatrixRecommendView) pageViews.get(3)).setAppInstallCount(appCount
				+ gameCount);}
