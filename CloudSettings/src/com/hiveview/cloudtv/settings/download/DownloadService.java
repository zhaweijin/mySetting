...
public class DownloadService extends Service {
	...
	private ArrayList<Long> mTempSpeed = new ArrayList<Long>();
	...
	private void startDownloadThread(String softId) {
		if (mDownloadTask == null) {
			...
			mDownloadListener = new DownloadListener();
			Log.d(TAG, " --- DownloadTask not null");
			mDownloadTask = new DownloadTask(this, mDownloadListener,isGroup);
			new Thread(mDownloadTask).start();
		}
	}
	...
	public class DownloadListener implements TaskListener {
		...
		public void taskProgress(Task task, long value, long max) {
			...
			mTempSpeed.add(value);
			int div = DownloadTask.DOWNLOAD_TIME/10;
			int size = mTempSpeed.size();
			if(size%div==0){
				//send
				long total=mTempSpeed.get(size-1);
				for(int i=1;i<=div;i++){
					if(total<mTempSpeed.get(size-i)){
						total = mTempSpeed.get(size-i);
					}
				}
