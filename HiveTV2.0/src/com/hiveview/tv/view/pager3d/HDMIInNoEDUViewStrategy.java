...
import com.hiveview.tv.view.MatrixVipBluelightView;
...
import com.hiveview.tv.view.pager3d.NoneHDMIInViewStrategy.CinemaFocusDirectioner;
import com.hiveview.tv.view.pager3d.NoneHDMIInViewStrategy.EDU;
...
@SuppressLint("NewApi")
public class HDMIInNoEDUViewStrategy extends Abstract3DPagerStrategy {
	...
	MatrixVipBluelightView vipBluelightView ;
	/**
	...
	//教育左边推荐位,应用左推荐位
	private View matrix_education_layout_1,matrix_app_layout_0;
	public HDMIInNoEDUViewStrategy(View3DPager _3dPager, OnDataCompleted dataCompleted) {
		super(_3dPager);

		appView = new MatrixAppView(view3dPager.getContext(), new AppFocusDirectioner());
		vipBluelightView= new MatrixVipBluelightView(view3dPager.getContext(), new BluelightFocusDirectioner());
		if(null == cinemaView){
		...
		vipBluelightView.setOnDataCompleted(dataCompleted);
		...
		onDataComplet = new OnDataComplet() {
			...
			public void getCinemaLayout_11(int isVisible_11) {
				// TODO Auto-generated method stub
				Log.v(HDMIInNoEDUViewStrategy.class.getSimpleName(), "isVisible_11=========="+(isVisible_11 == View.VISIBLE) );
				matrix_app_layout_0
						.setNextFocusLeftId((isVisible_11==View.VISIBLE)?R.id.matrix_cinema_layout_11_new_v:R.id.matrix_cinema_layout_11_new_h21);
			}
		...
		vipBluelightView.setVisibility(View.INVISIBLE);
		...
		view3dPager.addTabPageView(cinemaView, 1);
		view3dPager.addTabPageView(recommendView, 2);
		view3dPager.addTabPageView(vipBluelightView, 3);
	...
	public void setUnregister(){
		...
		vipBluelightView.setUnregister();
	}
			...
			if (null != views) {
				try {
					if (AppConstant.ISDOMESTIC) {
						...
						if (null != views[4]) {
							views[4].setNextFocusRightId(R.id.matrix_recommend_layout_0);
						}
						if (null != views[5]) {
							views[5].setNextFocusRightId(R.id.thumb_layout_0);
						}
					} else {
...
//			appExternalLayout.setNextFocusRightId(R.id.sub_navigation_common_game_record_text_layout);
	...
	class RecommendFocusDirectioner implements RecommendViewFocusDirectionListener {
		...
		public void setButtomMenuViewFocusDirection(View[] views) {
			...
			//首页左侧向左焦点运动
			if (AppConstant.ISDOMESTIC) {//国内版本
		...
		public void setRecommendViewsFocusDirection(View[] views) {
			if (null != views) {
				if (!AppConstant.ISDOMESTIC) {
					...
					views[0].setNextFocusLeftId(R.id.matrix_tv_layout_4);
					views[1].setNextFocusLeftId(R.id.matrix_tv_layout_4);
				}
			}
		}
	...
	public void setInstallApkCount(int gameCount, int appCount) {
		((MatrixRecommendView) pageViews.get(2)).setAppInstallCount(appCount+gameCount);
