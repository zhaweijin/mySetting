...
public class WifiModifyActivity extends Activity {
	...
	private static final int WIFI_MODIFY_ITEM_IPV4_MANUAL_ADJUST = 0;//2;
	private static final int WIFI_MODIFY_ITEM_IPV4_IP_ADDRESS = 1;//3;
	private static final int WIFI_MODIFY_ITEM_IPV4_NET_MASK = 2;//4;
	private static final int WIFI_MODIFY_ITEM_IPV4_GATEWAY = 3;//5;
	private static final int WIFI_MODIFY_ITEM_IPV4_DNS = 4;//6;
	...
	private static final int WIFI_MODIFY_ITEM_DISABLE_NET = 5;//13;
...
/*				if(DEBUG){
	...
	protected void onPause() {
		...
		processIpSettings(mIPv4Assignment==WifiAccessPoint.STATIC?true:false);
		...
		bundle.putParcelable(WifiInfoConfigEntity.KEY2, mIpConfiguration);
...
//			WifiModifyActivity.this.setResult(WifiInfoConfigEntity.RESULT_WIFI_MODIFY_OK, intent);   //RESULT_OK是返回状态码  
	...
	AdapterView.OnItemClickListener mItemListOnItemClickListener = new AdapterView.OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> arg0, View view, int position,
			...
			if(WIFI_MODIFY_ITEM_DISABLE_NET == position){
			...
			if(mItemCurEntity!=null && 
				...
				if(mIPv4Assignment == WifiAccessPoint.STATIC){
					if(position == WIFI_MODIFY_ITEM_IPV4_NET_MASK){
...
/*			if(DEBUG){
			...
			if(event.getAction() == KeyEvent.ACTION_DOWN){
				if(keyCode == KeyEvent.KEYCODE_DPAD_CENTER){
					...
					mItemCurHolder.mItemContentEdit.setFocusable(false);
					mWifiItemList.setSelectionFromTop(mItemListCurPosition-1, mWifiItemList.getSelectedView().getHeight());
				}
			}
			return false;
		}
	...
	private int processIpSettings(boolean hasIpSettings) {
        ...
        if (hasIpSettings) {
            ...
            try {
            	Inet4Address maskAddress = (Inet4Address) NetworkUtils.numericToInetAddress(mV4NetmaskAddress);
                int mask = NetworkUtils.inetAddressToInt(maskAddress);
            	networkPrefixLength = NetworkUtils.netmaskIntToPrefixLength(mask);
            ...
            //gateway
            String gateway = mV4GatewayAddress;
