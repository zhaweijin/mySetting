...
import com.hiveview.cloudtv.common.GeneraAdapter;
	...
	private void onBlindListener() {
		mOtherSettingListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				// 鍥惧儚
				if (position == 1) {
					startActivity(new Intent().setClass(mContext, ImageActivity.class));
				}
				if (position == 2) {
				...
				if (position == 5) {
					ToastUtils.showToast(getApplicationContext(), position+"", 0);
