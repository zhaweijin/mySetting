...
import com.hiveview.tv.view.television.OnLiveEpgPageItem.ONLiveFoursInterface;

public class OnliveEpg extends FrameLayout implements ONLiveFoursInterface {
	...
	public OnliveEpg(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
	}
	 ...
	 */
	private void init() {
		vpSelectEpg = (HivePreloadViewPager) this.findViewById(R.id.vp_epg_select);
		// 左右翻页
...
//		Log.i(TAG, "/registerReceiver....................................." + isegisterReceiver);
	...
	private void getChannelsByNames() {
		mCurrentChannelName = SwitchChannelUtils.getCurrentChannelName(mContext);
		HttpTaskManager.getInstance().submit(new Runnable() {
			@Override
			public void run() {
	...
	OnLiveEpgPageItem pageView = null;
	...
	private void setChannelCode() {
		if (null != channelList && channelList.size() > 0) {
			...
			((OnlivePlayerActivity) mContext).setCHANNEL_LOGO(channelEntity.getLogo());
			((OnlivePlayerActivity) mContext).setCHANNEL_NAME(channelEntity.getName());
	...
	public void getTheOneView() {
		...
		pageView = (OnLiveEpgPageItem) hash.get(pageIndex);
		OnLiveEpgPageItem pageViewNow = (OnLiveEpgPageItem) mAdapter.getCurrentPageView();
	...
	private void setData() {
		...
		if (null == mAdapter) {
			mAdapter = new HivePagerAdapter(mContext,
					new OnliveEpgPageViewFactory(new ItemViewFocusListener(), new ItemCallBackItemViewKeyListener()), vpSelectEpg, 7,
	...
	public static void getEpgPage(ONLiveEpgInterface onLiveEpgInterface) {
		/*
		 * start by guosongsheng CHANNEL_CODE参数传空在new LiveEpgInfoFragment
		 * 的时候会将中的LiveEpgInfoFragment CHANNEL_CODE制空
		 */
		onLiveEpgInterface.getEpgPage(new OnliveEpg(mContext, null));
		/* end by guosongsheng */
	}
	public interface ONLiveEpgInterface {
		public void getEpgPage(ONLiveFoursInterface foursInterface);
	}
