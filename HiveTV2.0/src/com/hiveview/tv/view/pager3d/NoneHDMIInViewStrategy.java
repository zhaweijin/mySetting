...
@SuppressLint("NewApi")
public class NoneHDMIInViewStrategy extends Abstract3DPagerStrategy {
	...
	// 首页右边推荐位
	private View matrix_recommend_layout_1, matrix_recommend_layout_3,
			matrix_recommend_layout_4;

	public NoneHDMIInViewStrategy(View3DPager _3dPager,
			OnDataCompleted dataCompleted) {
		super(_3dPager);
		MatrixAppView appView = new MatrixAppView(view3dPager.getContext(),
				new AppFocusDirectioner());
		MatrixBluelightView bluelightView = new MatrixBluelightView(
				view3dPager.getContext(), new BluelightFocusDirectioner());
		if (null == cinemaView) {
			cinemaView = new MatrixCinemaViewNew(view3dPager.getContext(),
					new CinemaFocusDirectioner());
		}
		MatrixRecommendView recommendView = new MatrixRecommendView(
				view3dPager.getContext(), new RecommendFocusDirectioner());
		MatrixGameView gameView = new MatrixGameView(view3dPager.getContext(),
				new GameFocusDirectioner());
		MatrixEducationView educationView = new MatrixEducationView(
				view3dPager.getContext(), new EDU());
		/* start by guosongsheng 美国测试使用 */
		// TabBasePageView tvView = null;
		/*
		 * if (AppConstant.ISFOREIDGN) { tvView = new
		 * MatrixTVView(view3dPager.getContext(), new TvFocusDirectioner()); }
		 * else { tvView = new MatrixTVForeidgnView(view3dPager.getContext(),
		 * new TvFocusDirectioner()); }
		 */
		/* end by guosongsheng */
		...
		onDataComplet = new OnDataComplet() {
			...
			public void getCinemaLayout_6(int l6) {
				...
				matrix_recommend_layout_1
						.setNextFocusRightId((l6 == View.VISIBLE) ? R.id.matrix_cinema_layout_6_new_v
								: R.id.matrix_cinema_layout_6_new_h12);
				matrix_recommend_layout_4
						.setNextFocusRightId((l6 == View.VISIBLE) ? R.id.matrix_cinema_layout_6_new_v
								: R.id.matrix_cinema_layout_6_new_h11);
			}
			...
			public void getCinemaLayout_1(int l1) {
				...
				matrix_recommend_layout_3
						.setNextFocusRightId((l1 == View.VISIBLE) ? R.id.matrix_cinema_layout_1_new_v
			...
			public void getCinemaLayout_11(int isVisible_11) {
				// TODO Auto-generated method stub
				matrix_education_layout_1
						.setNextFocusLeftId((isVisible_11 == View.VISIBLE) ? R.id.matrix_cinema_layout_11_new_v
								: R.id.matrix_cinema_layout_11_new_h21);
			}
		...
		// view3dPager.addTabPageView(tvView, 4);
		view3dPager.addTabPageView(gameView, 5);
	}
	...
	/**
	 * @ClassName: GameFocusDirectioner
	 * @Description: TODO
	 * @author: zhangpengzhan
	 * @date 2014年9月17日 下午1:13:35
	 * 
	 */
	class GameFocusDirectioner implements RecommendViewFocusDirectionListener {
		...
		public void setRecommendViewsFocusDirection(View[] views) {
			if (null != views) {
				if (!AppConstant.ISDOMESTIC) {
					...
					views[1].setNextFocusRightId(R.id.matrix_tv_layout_3);
		...
		public void setButtomMenuViewFocusDirection(View[] views) {
			...
			gameRecordLayout
					.setNextFocusLeftId(R.id.sub_navigation_common_app_external_text_layout);
			// 观看记录
			View gameExternalLayout = views[views.length - 1];
			gameExternalLayout
					.setNextFocusRightId(R.id.sub_navigation_common_bluelight_record_text_layout);
			views[1].setNextFocusRightId(R.id.matrix_bluelight_layout_0);
			views[2].setNextFocusRightId(R.id.matrix_bluelight_layout_2);
			views[3].setNextFocusRightId(R.id.matrix_bluelight_layout_2);
		}
	...
	/**
	 * @ClassName: TvFocusDirectioner
	 * @Description: 电视底部菜单栏焦点走向控制
	 * @author: zhangpengzhan
	 * @date 2014年9月17日 下午1:13:07
	 * 
	 */
	class TvFocusDirectioner implements RecommendViewFocusDirectionListener {
		...
		public void setButtomMenuViewFocusDirection(View[] views) {
			...
			categoryView
					.setNextFocusLeftId(R.id.sub_navigation_game_setting_text_layout);
			...
			if (!AppConstant.ISDOMESTIC) {
				...
				settingView
						.setNextFocusLeftId(R.id.sub_navigation_game_setting_text_layout);
				settingView
						.setNextFocusRightId(R.id.sub_navigation_recommend_record_text_layout);
				/* end by guosongsheng */
			} else {
				// 直播提醒
	...
	/**
	 * @ClassName: AppFocusDirectioner
	 * @Description: 应用底部菜单栏焦点走向控制
	 * @author: zhangpengzhan
	 * @date 2014年9月17日 下午1:12:36
	 * 
	 */
...
//			appRecordLayout
//					.setNextFocusLeftId(R.id.sub_navigation_common_cinema_external_text_layout);
//
//			// 外接媒体
//			View appExternalLayout = views[views.length - 1];
//			appExternalLayout
//					.setNextFocusRightId(R.id.sub_navigation_common_game_record_text_layout);
	...
	/**
	 * @ClassName: RecommendFocusDirectioner
	 * @Description: 首页底部菜单栏翻页时候焦点走向
	 * @author: zhangpengzhan
	 * @date 2014年9月17日 下午1:10:52
	 * 
	 */
	class RecommendFocusDirectioner implements
		...
		public void setButtomMenuViewFocusDirection(View[] views) {
			...
			recommendFavouriteLayout
					.setNextFocusLeftId(R.id.sub_navigation_common_bluelight_external_text_layout);
			// 外接存储
			View recommendExternalLayout = views[3];
			recommendExternalLayout
					.setNextFocusRightId(R.id.sub_navigation_common_cinema_record_text_layout);
			// 首页左侧向左焦点运动
	...
	/**
	 * @ClassName: CinemaFocusDirectioner
	 * @Description: 影院底部菜单栏焦点走向
	 * @author: zhangpengzhan
	 * @date 2014年9月17日 下午1:11:23
	 * 
	 */
	class CinemaFocusDirectioner implements RecommendViewFocusDirectionListener {
		...
		public void setButtomMenuViewFocusDirection(View[] views) {
			...
			movieRecordLayout
					.setNextFocusLeftId(R.id.sub_navigation_common_recommend_external_text_layout);

			// 外接存储
			View movieExternalLayout = views[views.length - 1];
			movieExternalLayout
					.setNextFocusRightId(R.id.sub_navigation_common_app_record_text_layout);
		}
	...
	/**
	 * @ClassName: BluelightFocusDirectioner
	 * @Description:蓝光底部菜单栏焦点控制
	 * @author: zhangpengzhan
	 * @date 2014年9月17日 下午1:11:52
	 * 
	 */
	class BluelightFocusDirectioner implements
			RecommendViewFocusDirectionListener {
		...
		public void setButtomMenuViewFocusDirection(View[] views) {
			...
			if (AppConstant.ISDOMESTIC) {
				bluelightRecordLayout
						.setNextFocusLeftId(R.id.sub_navigation_common_game_external_text_layout);
			} else {
				// 外接媒体
				bluelightRecordLayout
						.setNextFocusLeftId(R.id.sub_navigation_common_tv_external_text_layout);
			}
			// 影片收藏
			View bluelightExternalLayout = views[views.length - 1];
			bluelightExternalLayout
					.setNextFocusRightId(R.id.sub_navigation_common_recommend_favourite_text_layout);

			views[1].setNextFocusLeftId(R.id.matrix_game_layout_4);
			views[2].setNextFocusLeftId(R.id.matrix_game_layout_5);
	...
	public void setInstallApkCount(int gameCount, int appCount) {
		((MatrixRecommendView) pageViews.get(3))
				.setAppInstallCount(appCount + gameCount);
	}
