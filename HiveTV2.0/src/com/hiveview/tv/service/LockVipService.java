...
public class LockVipService extends Service {
	
	private String TAG="LockVipService";
	/**
	 * 访问接口成功
	 */
	 ...
	 * 锁网弹出框View
	 */
	private LockVipDialogView lockNetDialogView = null;
	...
	public void onCreate() {
		super.onCreate();
		Log.i(TAG, "GET_SUO_SERVICE onCreate ....................");
	...
	private void getLockVip() {
		new HttpTaskManager().submit(new SafeRunnable() {
			...
			public void requestData() {
				...
				Timestamp=time.format(nowDate);
				//加密取得sign
				parameterMap.put("MacAddress", DeviceBoxUtils.getMac());
				parameterMap.put("SequenceNo", DeviceBoxUtils.getSn());
				parameterMap.put("Timestamp", Timestamp);
				Sign=SignUtils.sign(parameterMap, SignUtils.shardedKey);
				
				ArrayList<VipLockEntity> appMarketEntities = new HiveTVService()
						.getLockVip(mContext,null,
								DeviceBoxUtils.getMac(), DeviceBoxUtils.getSn(),Timestamp,Sign);
				Log.i(TAG, "suo..................start2:"
						+ appMarketEntities.toString());
				if (null != appMarketEntities && appMarketEntities.size() > 0) {
					boolean isLockNet = appMarketEntities.get(0).isStatus();
					Log.i(TAG, "suo..................start3：" + isLockNet);
					if (!isLockNet) {
						Log.i(TAG, "GET_SUO..................success"
								+ isLockNet);
						handler.sendEmptyMessage(GET_NET_LOCK_SUCCESS);
					} else {
						Log.i(TAG, "GGET_SUO_FAIL..................fail:"
								+ isLockNet);
...
/*				// 盒子网络故障，应该弹出设置网络的提示框
	...
	protected void isShowLockNetDialog() {
		if (null == mWm) {
			...
			lockNetDialogView = new LockVipDialogView(this, null);
