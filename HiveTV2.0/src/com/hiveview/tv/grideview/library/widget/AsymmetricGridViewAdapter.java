...
import com.hiveview.tv.grideview.library.AsymmetricGridViewAdapterContract;
import com.hiveview.tv.grideview.library.AsyncTaskCompat;
import com.hiveview.tv.grideview.library.Utils;
...
public abstract class AsymmetricGridViewAdapter<T extends AsymmetricItem>
    ...
    public View getView(final int position, final View convertView, final ViewGroup parent) {
        ...
        while (!rowItems.isEmpty() && columnIndex < listView.getNumColumns()) {
            ...
            if (spaceLeftInColumn >= currentItem.getRowSpan()) {
                ...
                childLayout.setFocusable(false);
                childLayout.setFocusableInTouchMode(false);
                ...
                v.setLayoutParams(new LinearLayout.LayoutParams(130*currentItem.getColumnSpan(),60));
    ...
    private IcsLinearLayout findOrInitializeLayout(final View convertView) {
        ...
        for (int j = 0; j < layout.getChildCount(); j++) {
            ...
            tempChild.setFocusableInTouchMode(false);
    ...
    private RowInfo<T> calculateItemsForRow(List<T> items, float initialSpaceLeft) {
    	 final List<T> itemsThatFit = new ArrayList<T>();
         int currentItem = 0;
         int rowHeight = 1;
         float areaLeft = 6;
         if (listView.isDebugging())
             Log.d(TAG, String.format("areaLeft0::%s", areaLeft));
         while (areaLeft > 0 && currentItem < items.size()) {
             ...
             /*if (rowHeight < item.getColumnSpan()) {
                 ...
                 areaLeft -= itemArea;
                 if (listView.isDebugging())
                     Log.d(TAG, String.format("areaLeft1::%s", areaLeft));
