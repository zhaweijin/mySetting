 ...
 */
@SuppressLint("NewApi")
public class LauncherFocusViewH extends RelativeLayout {
    /**
     * 焦点动画移动的时间
     */
    ...
    /*
     * add resolution_offset_width resolution_offset_x
     * 是为了统一720p 1080p 焦点框宽带在wifi 分享、pppoe拨号界面错乱的问题
     */
    public int resolution_offset_width = 0;
    public int resolution_offset_x = 0;

    public int resolution_offset_height = 0;
    public int resolution_offset_h = 0;
    ...
    public LauncherFocusViewH(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        mContext = context;
        init();
    }

    public LauncherFocusViewH(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        init();
    }

    public LauncherFocusViewH(Context context) {
        super(context);
        mContext = context;
        init();
    }
    ...
    public void initFocusView(View firstCallRequestFocusOfView, boolean isScale, float scale) {
        ...
        // 设置焦点View的在屏幕中的坐标
        setX(targetX - xyOffset - outXOffset-resolution_offset_x);
        setY(targetY - xyOffset - outYOffset-resolution_offset_h);

//        Log.v(TAG, "targetX="+targetX+",targetY="+targetY+",w="
//        +firstCallRequestFocusOfView.getWidth()+",h="+firstCallRequestFocusOfView.getHeight());
        viewWrapper.setWidth(firstCallRequestFocusOfView.getWidth() + wOffset+resolution_offset_width);
        viewWrapper.setHeight(firstCallRequestFocusOfView.getHeight() + hOffset+resolution_offset_height);
        ...
        vpaXY = animate().x(targetX - xyOffset - outXOffset-resolution_offset_x).y(targetY - xyOffset - outYOffset-resolution_offset_h)
    ...
    public void moveTo(View tagetView) {
        ...
        targetWidth = targetWidth + resolution_offset_width;
        targetHeight = targetHeight + resolution_offset_height;

        Log.e(TAG, "huxing------targetWidth = " + targetWidth);
        Log.e(TAG, "huxing------targetHeight = " + targetHeight);
        ...
        // 计算焦点框View在屏幕中的坐标
        int x = targetX - xyOffset - outXOffset-resolution_offset_x;
        int y = targetY - xyOffset - outYOffset-resolution_offset_h;
        Log.e(TAG, "x===="+x + ",y===="+y);
    ...
    public void moveToWifiTop(View tagetView) {
        ...
        int targetWidth = mContext.getResources().getDimensionPixelSize(R.dimen.wifi_move_to_top_width);
        int targetHeight = mContext.getResources().getDimensionPixelSize(R.dimen.wifi_move_to_top_height);
        ...
        Log.e(TAG, "x = " + x+"  y=="+y);
        x = mContext.getResources().getDimensionPixelSize(R.dimen.wifi_move_to_top_x);
        y = mContext.getResources().getDimensionPixelSize(R.dimen.wifi_move_to_top_y);
     ...
     *
     */
    class FocusViewMoveListener implements Animator.AnimatorListener {
