 ...
 *
 */
public class QiYiPlayerVoiceController implements IVoiceController {

	private static final String TAG = "QiYiPlayerVoiceController";
	private static String[] playerCommands = {"voice_increase","voice_decrease","voice_close","voice_open","PLAY", 
		"PAUSE", "RESUME", "RESTART", "SEEK", "FORWARD", "BACKWARD", "EXIT", "PREV", "NEXT", "INDEX"};
	...
	public void control(final Context context, Intent intent) {
		...
		int episode = intent.getIntExtra(PredefineSemantic.PARAM_EPISODE, -1);
		...
		if (command != null && command.equals("voice_increase")) {
			...
			Intent broad = new Intent(PlayerBroadCastContants.ACTION_VOICE_QIYI_PLAYER);
		...
		if (null != action && action.equals(PredefineSemantic.ACTION_PREVIOUS)) {
			speaker.showText(context.getString(R.string.episode_previous), true);
			Intent broad = new Intent(PlayerBroadCastContants.ACTION_VOICE_QIYI_PLAYER);
			broad.putExtra(PlayerBroadCastContants.PLAYER_CONTROL, PlayerBroadCastContants.PLAYER_PREVIOUS);
			broad.putExtra(PlayerBroadCastContants.PLAYER_TIME, offset);
			context.sendBroadcast(broad);
		}
		if (null != action && action.equals(PredefineSemantic.ACTION_NEXT)) {
			speaker.showText(context.getString(R.string.episode_next), true);
			Intent broad = new Intent(PlayerBroadCastContants.ACTION_VOICE_QIYI_PLAYER);
			broad.putExtra(PlayerBroadCastContants.PLAYER_CONTROL, PlayerBroadCastContants.PLAYER_NEXT);
			broad.putExtra(PlayerBroadCastContants.PLAYER_TIME, offset);
			context.sendBroadcast(broad);
		}
		if (null != action && action.equals(PredefineSemantic.ACTION_INDEX)) {
			speaker.showText(context.getString(R.string.episode_previous), true);
			Intent broad = new Intent(PlayerBroadCastContants.ACTION_VOICE_QIYI_PLAYER);
			broad.putExtra(PlayerBroadCastContants.PLAYER_CONTROL, PlayerBroadCastContants.PLAYER_INDEX);
			broad.putExtra(PlayerBroadCastContants.PLAYER_EPISODE, episode);
			context.sendBroadcast(broad);
		}
	...
	public static boolean isFaceToPlayer(Context context) {
		...
		if (className.equalsIgnoreCase(AppScene.QIYI_PLAYER)) {
			return true;
		}
		return false;
	}
