...
public class LiveFastSelectTvChannelActivitys extends BaseActivity {
	...
	private static String RECEIVER_ACTION = "com.hiveview.tv.activity.LiveFastSelectTvChannelActivity";
	...
	protected void processData(int msgWhat) {
		...
		case REQUEST_CCTV_CHANNEL_SUCCESS:// 央视频道
			...
			mCctvAdapter = new HivePagerAdapter(this, new TvChannelPageViewFactory(new ItemViewFocusListener(), new ItemViewKeyListener()), cctvViewPager, PAGE_SIZE,
			...
			// 初始化央视是否显示向右翻页的图片
			if (PAGE_SIZE < cctvChannelList.size()) {
				ivPagerRight.setVisibility(View.VISIBLE);
			}
		...
		case REQUEST_TV_CHANNEL_SUCCESS:// 卫视频道
			...
			tvAdapter = new HivePagerAdapter(this, new TvChannelPageViewFactory(new ItemViewFocusListener(), new ItemViewKeyListener()), tvViewPager, PAGE_SIZE,
					new NeighborTwoLineStrategy());
		...
		case REQUEST_LOCAL_CHANNEL_SUCCESS:// 本地频道
			...
			localAdapter = new HivePagerAdapter(this, new TvChannelPageViewFactory(new ItemViewFocusListener(), new ItemViewKeyListener()), localViewPager, PAGE_SIZE,
					new NeighborTwoLineStrategy());
		...
		case REQUEST_HD_CHANNEL_SUCCESS:// 高清频道
			...
			hdAdapter = new HivePagerAdapter(this, new TvChannelPageViewFactory(new ItemViewFocusListener(), new ItemViewKeyListener()), hdViewPager, PAGE_SIZE,
					new NeighborTwoLineStrategy());
		...
		case REQUEST_PAY_CHANNEL_SUCCESS:// 收费频道
			...
			payAdapter = new HivePagerAdapter(this, new TvChannelPageViewFactory(new ItemViewFocusListener(), new ItemViewKeyListener()), payViewPager, PAGE_SIZE,
					new NeighborTwoLineStrategy());
	...
	class ItemViewFocusListener implements CallBackItemViewFocusListener {
		...
		public void onCallBackFocusChange(View view, boolean has) {
			...
			if (has) {
				...
				if (entity.getPositionInItemView() < PAGE_SIZE / 2) {
	...
	class ItemViewKeyListener implements CallBackItemViewKeyListener {
		...
		public boolean onKey(View v, int keyCode, KeyEvent event) {
			...
			if (keyCode == KeyEvent.KEYCODE_DPAD_RIGHT && event.getAction() == KeyEvent.ACTION_DOWN) {
				...
				if (viewPagers[mShowViewPagerIndex].getCurrentItem() == adapter.getLocalPageTotal() - 1 && (entity.getPositionInItemView() + 1) == adapter.getLocalDataSize() % 10) {
					return true;
				}
			}
			
			if(keyCode == KEYCODE && event.getAction() == KeyEvent.ACTION_DOWN){
				SwitchChannelUtils.switchChannel(LiveFastSelectTvChannelActivitys.this, entity.getName(), false, AppScene.getScene());
				Log.i(TAG, "获取的台的名称====>" + entity.getName());
				Intent intent = new Intent();
				intent.setAction(RECEIVER_ACTION);
				//要发送的内容
				intent.putExtra("author", entity.getCode());
				//发送 一个无序广播
