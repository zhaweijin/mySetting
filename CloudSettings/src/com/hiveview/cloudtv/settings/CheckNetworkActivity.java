...
public class CheckNetworkActivity extends Activity implements HttpCheckListener{
	...
	private static final int CHECK_NETWORK_FINISHED = 1006;
	...
	private ImageView check_network_local;
		...
		mHandler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				case CHECK_NETEORK_ROUTER_TO_INTERNET_FAILED:
					...
					break;
				case CHECK_NETWORK_FINISHED:
					Log.v("aa", "check finished");
					for (int i = 0; i < 10; i++) {
						mRouter2InternetUnSelectedImg[i].setVisibility(View.GONE);
						mRouter2InternetSelectedImg[i].setVisibility(View.VISIBLE);
					}
					break;
				default:
					break;
	...
	protected void onResume() {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					if (type == ConnectivityManager.TYPE_WIFI || type == SettingsApplication.TYPE_PPPOE
					...
					mHandler.sendEmptyMessage(CHECK_NETWORK_FINISHED);
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					Thread.sleep(5000);
					new HttpCheck(CheckNetworkActivity.this);
	...
	private void initView() {
		...
		check_network_local = (ImageView)findViewById(R.id.check_network_local);
		check_network_local.setBackgroundResource(R.drawable.box);
