 ...
 * 
 */
public class MovieListItemPageView extends HiveBaseView {

	protected static final String TAG = "MovieListItemPageView";
	private View item1;
	private View item2;
	private View item3;
	private View item4;
	...
	public MovieListItemPageView(Context context, AttributeSet attrs) {
		super(context, attrs);
		this.context = context;
	}

	public MovieListItemPageView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		this.context = context;
	}

	public MovieListItemPageView(Context context, int everyPageSize, int everyPageLine) {
	...
	class ItemFocusChangeListener implements OnFocusChangeListener {
		...
		public void onFocusChange(View arg0, boolean arg1) {
			if (null != focusChangeListeners) {
				focusChangeListeners.viewFocusListener(v, arg1);
			}
	 ...
	 * @param ItemView
	 */
	private void setTextData(TextView tvUpdate, TextView tv, FilmNewEntity entity, View ItemView) {
		tv.setText(entity.getName());
		...
		if (show_type == -1) {
			...
			ArrayList<FirstClassListEntity> arrayList = channelDAO.query(new String[] { "show_type" }, "firstclass_id = ?",
					new String[] { String.valueOf(entity.getCid()) }, null);
			FirstClassListEntity firstClassListEntity = arrayList.get(0);
			show_type = firstClassListEntity.getShow_type();
			firstclassId = firstClassListEntity.getFirstclass_id();
			Log.d(VIEW_LOG_TAG, "arrayList::" + arrayList + "|||| show_type::" + show_type);
		}

		if (show_type == ContentShowType.TYPE_MULTIPLE_VIDEO_DETAIL) {
			if (firstclassId == 1) {//判断是否为电影
				tvUpdate.setVisibility(View.INVISIBLE);
			} else {
				if (allSize == updateSize) {
					tvUpdate.setText(
							String.format(getResources
									  ().getString(R.string.search_landscape_time_total), allSize));
				} else {
					if(updateSize==0){
						tvUpdate.setText("");
					}else{
						tvUpdate.setText(
								String.format(getResources
										  ().getString(R.string.search_landscape_time_update), entity.getCurrCount()));
					}
				}
			}
		} else {
	...
	private void displayImage(final ImageView imageView, final FilmNewEntity entity) {
		...
		if (null != entity && null != entity.getPosterUrl() && !entity.getPosterUrl().equals("") && !entity.getPosterUrl().equals("null")) {
			imageLoader.displayImage(ToolsUtils.createImgUrl(entity.getPosterUrl(), true), imageView, optionsPoster,new ImageLoadingListener() {
				...
				public void onLoadingFailed(String arg0, View arg1, FailReason arg2) {
					...
					imageLoader.displayImage(ToolsUtils.createImgUrl(entity.getPicUrl(), true), imageView, optionsPoster);
					}}
